...
举个例子，这是采用敦煌风格迁移做的服饰图片：
![Image text](https://github.com/JChrysanthemum/deepflying/blob/master/InnoCamp/SF/pics/content.png)
这是魔法还是深度学习?幸运的是，这并不涉及任何魔法:风格迁移是一个非常有趣的技术，它展示了神经网络的功能及内部表示。
神经风格传递的原理是定义两个距离函数。一个是Lstyle它描述了两幅图像的内容有多么不同，另一个是Lstyle，它描述了两幅图像的风格差异。然后，给三幅图像，一个期望的风格图像、一个期望的内容图像和一张输入图像(用内容图像初始化)，我们尝试对输入图像进行转换，来最小化与内容图像的内容距离及其与样式图像的样式距离。总之，我们将获取基本输入图像、要匹配的内容图像和要匹配的样式图像。我们用反向传播算法最小化内容和样式距离(损失)来转换基本输入图像，创建一个匹配内容图像的内容和样式图像的样式的图像。
