...
# Use GANS/tools/getImgsPath.py to generate this txt file first
# This "/home/xxxx" is a Linux style path , just use your origin path-style for WIN or MAC
IMG_DATA_PATH = '/home/deepcam/Data/Jeans.txt'
...
class DCGAN():
    ...
    def train(self, epochs, batch_size=128, save_interval=50,chunk_size=3000):
        ...
        for chunk in reader:
            ...
            for epoch in range(epochs):
                ...
                # print ("\r%d [D loss: %f, acc.: %.2f%%] [G loss: %f]" % (epoch, d_loss[0], 100*d_loss[1], g_loss))

                # If at save interval => save generated image samples
                if epoch % save_interval == 0:
                    self.save_imgs(epoch)
        self.generator.save('models/dcgan_IMG_%d_%d_%d_%s_G.h5' % (IMG_ROWS,IMG_COLS,epochs,IMG_DATA_PATH.split('/')[-1].split(".")[0]))
        print('\n')
    ...
    def save_imgs(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig_name = "images/DCGAN_IMG/%s_%d.png" % (IMG_DATA_PATH.split('/')[-1].split('.')[0], epoch)
...
if __name__ == '__main__':
    dcgan = DCGAN()
    dcgan.train(epochs=40001, batch_size=64, save_interval=5000)
